...
```
All the C++ language syntax is detailed the official C++ Standard Reference Documentation [here](https://en.cppreference.com/w/), although it may look daunting, this reference does have some useful information for beginners and will become more accessible as you work through the basic concepts covered in this module.
...
The live code editor on the following pages, uses a C++ Kernel that runs in Jupyter-Notebook. This is not the usual way to work and has significant limits.
...
You might want to download the Community Edition (free) of Visual Studio in advance.
...
To enable portability the programmer should always understand be clear about what implementation-defined features are being used and to try to isolate implementation specific features to specific parts of the program - in separate files, functions etc. - then only these sections need be changed.
...
Objects are allocated to the free store using `new` and persist until specifically deleted. Failing to deallocate memory in the free store, during execution causes memory resource issues, `delete` must be used to deallocate to prevent a memory leak. 
...
Memory is a finite resource, and programmers often need to consider how objects are allocated into different types of memory to make it possible to run a program on a given device.
...
The amount of stack memory required depends on the size of the local variables be and the interaction of the functions that are called. A function that uses a large array and calls itself repeatedly will use up a lot of the available stack. Programs can usually by rewritten to minimise stack requirement - sometimes by increased static memory.

In C++, the most likely cause of unexpected "out of memory errors" is running out of memory on the heap because of not deallocating objects properly.
Memory leaks occur when memory remains allocated after it is no longer needed, due to the failure of the program to release memory back to the heap with delete when the object is no longer required (after a function has been executed). Leak in the sense that the program has a decreasing amount of memory available to it. Some languages handle this issue using a process called automatic garbage collection.
```
````
